...
let displayResults = (allResults) => {
    ...
    for (let i = 0; i < allResults.length; i++) {
        if (allResults[i].date === moment().format("YYYY-MM-DD")) {
            todaySteps.innerHTML = `<p class="result"/> (${moment().format(
                "DD.MM.YYYY"
            )}) ${allResults[i].amount} </p>`;
        }
        if (
            allResults[i].date ===
            moment().subtract(1, "days").format("YYYY-MM-DD")
        ) {
            yesterdaySteps.innerHTML = `<p class="result"/> (${moment()
                .subtract(1, "days")
                .format("DD.MM.YYYY")}) ${allResults[i].amount}</p>`;
        }
    }
    ...
    for (let i = 0; i < allResults.length; i++) {
        if (
            moment().format("YYYY-MM-DD") >= allResults[i].date &&
            allResults[i].date >
                moment().subtract(7, "days").format("YYYY-MM-DD")
        ) {
            data.labels.push(
                moment(allResults[i].date, "YYYY-MM-DD").format("dddd")
            );
            data.datasets[0].data.push(allResults[i].amount);
            x += allResults[i].amount;
            getId(
                "averageNumberSteps"
            ).innerHTML = `Average number of steps per week: ${Math.round(
                x / 7
            )}`;
        }
    }
    ...
    currentChart = new Chart(getId("showResults"), config);
};
...
document.addEventListener("DOMContentLoaded", () => {
    let validat = () => {
        if (
            (!isValid(date.value) || !isValid(inputNewResult.value)) === false
        ) {
            save.disabled = false;
        } else {
            save.disabled = true;
