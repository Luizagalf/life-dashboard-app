...
const handleError = () => {
    ...
    newsError.innerHTML += `<div class="box">
...
async function showNews() {
    try {
        const response = await fetch(
            "https://newsapi.org/v2/top-headlines?country=us&category=business&apiKey=8c256c8b1b714561ab13048f113bef4f"
        );

        const data = await response.json();
        let amount = Math.round(
            Math.random() * (data.articles.length - 8 - 0) + 0
        );
